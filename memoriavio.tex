...
\LARGE
\textbf{3D Charts visualizations for Kibana}
...
The plugin is hosted in the web-based Git repository hosting service GitHub, under open source
software license. It has a web page of presentation where more information and details of the
project are provided to the user. Inside the repository, where project?s software is hosted, there?s
documentation like installing and uninstalling the plugin

The Elastic team maintains a list of plugins developed by external developers. This plugin has been reviewed by this team and published on the official site. At the moment people is starting to use this plugin. Hundreds of people have viewed the project and a few dozens have downloaded the project.

\begin{itemize}
\item Project page: \url{https://virusu.github.io/3D_kibana_charts_vis/}
\item Github repository: \url{https://github.com/virusu/3D_kibana_charts_vis}
\end{itemize}
...
\begin{figure}
  \centering
  \includegraphics[height=8cm, keepaspectratio]{img/kibana_tabs}
...
AngularJS is not a single piece in the overall puzzle of building the client-side of a web application. It handles all of the DOM and AJAX glue code that was written by hand and puts it in a well-defined structure. This makes AngularJS opinionated about how a CRUD (Create, Read, Update, Delete) application should be built.
...
Kibana exploits AngularJS power in order to build its pages. Also, in order to develop this project, AngularJS and its main features had to be used (scopes, modules, directives,...).
...
In Figure \ref{fig:webglexample} an example of what can be done with WebGL is displayed.
...
Three.js is a cross-browser JavaScript library/API used to create and display animated 3D computer graphics in a web browser using WebGL. Three.js allows the creation of GPU-accelerated 3D animations using the JavaScript language as part of a website without relying on proprietary browser plugins.
...
\item Three.js runs in all browsers supported by WebGL.
\end{itemize}

In figure \ref{fig:threeexample} an example of what can be done with this library is shown. In particular, this a a cool racing game living in the browser and built entirely with three.js library.
...
In this project, all scenes are three.js scenes. This means that every 3D chart built in this project has its own three.js objects: scenes, cameras, lights, materials, objects and geometry. But this library doesn't include predefined 3D charts. These charts were used using an extension of three.js called threeDC.js, which is exposed in the next section.

\section{ThreeDC.js}

ThreeDC.js is a webGL Based 3D library for building 3D charts, developed during 2016 by an URJC student. It does not use webGL directly, but is built on top of three.js, which provides a higher level of abstraction.

ThreeDC provides easy usable 3D charts. The 3D charts it provides at the moment are: bars chart, line chart, pie chart, smooth curve chart, bubbles chart, scatter chart and panels to contain these charts. In Figure \ref{fig:threedcexample} an entire world of charts is shown, as a sample of this library's power. 
...
It is possible to move around in this world of data charts and even interact with them. When moving the mouse over an element, it shows info about that element. And when we click a certain element, all the other charts filter their data by that field we clicked on.

In this project threeDC has been used to integrate the 3D bars chart, pie chart and bubbles chart into Kibana.
...
In each sprint a Sprint Backlog is defined. It collects all the tasks that are needed to be done and who is going to do them, as well as an estimation for the time needed for each task to be completed. Figure \ref{fig:scrum-workflow} shows a typical Scrum workflow.
...
So whatever is put in an index.html file, the server serves it. We start creating and modifying this file, which you can completely see at the Appendix \ref{sec:index.html} of this paper. You can see the final result of this Sprint in Figure \ref{fig:index-ES-threeDC}.
...
For a detailed explanation on routes please consult the excellent Tim Roes tutorial about writing  custom applications, in which this sprint was based.
...
The final result can be seen in Figure \ref{fig:elasticsearch-app-ncommits}.
...
In this section, a very basic visualization type plugin in Kibana will be built from scratch. It is intended to make this plugin as simple as it is possible, for example only showing a "Hello World" message.
...
\item sprint2.js. In this file, we define our visualization setting parameters like a title, a description or an icon to appear in visualization list. We also define here the controller for our div in Kibana, which we will describe in the next paragraph. Full code can be seen in Appendix \ref{sec:sprint2.js}.
...
In figure \ref{fig:3dcharts-architecture} the final architecture of this plugin is displayed. All these files, with this structure, can be found in the github project\footnote{\url{https://github.com/virusu/3D_kibana_charts_vis}}. As it has been previously said, a Kibana plugin has a predefined structure. Next, we will explain what the different files are, categorized by color:
...
\begin{itemize}
\item \textbf{package.json} is necessary in order to define the npm package. As it has been said, the plugin is a npm package itself. This file contains info such as name, version and dependency of other packages.
\item \textbf{index.js} is the npm module itself and setups everything the plugin needs. A new instance of Kibana must be created in this file and it also specifies that this plugin is a visualization type plugin.
\item \textbf{vr\_vis.js} is the main file that defines the visualization and launches everything else. It has general information such as  visualization icon and where to find the templates, each template and each controller for each template.
\item \textbf{pie3D.js}, \textbf{pie3Dcontroller.js} and \textbf{pie3D.html} define the behaviour of the 3D pie. The html file contains the template, the .js main file contains the visualization definition and then we have the controller living in its own folder.
\item \textbf{bars3D.js}, \textbf{bars3Dcontroller.js} and \textbf{bars3D.html}: they define all the behaviour for the 3D bars.
\item \textbf{bubbles3D.js}, \textbf{bubbles3Dcontroller.js} and \textbf{bubbles3D.html}: they define all the behaviour for the 3D bubbles.
\item \textbf{threedc} folder contains all that has to do with the graphics. Most importantly, it contains the threeDC librry itself. Then, it contains all the extensions that are necessary for this library to work properly.
\end{itemize}
...
We may have reached our main goals, but there is still work that can be done to improve this project. I will mention some guidelines on some new or improved features:

\begin{itemize}

\item Add some more 3D charts
\item Work with the scene dimensions in order to adapt it to each user screen, as well as playing with the size and centering of the graphs to adjust them perfectly when initializing them in a Kibana dashboard
...
\item Investigate on improving performance. At the moment, when displaying more than one scene specially, the dashboard begins to move quite slowly when having a lot of data.
\end{itemize}
